...
sub handlePopup {
    ...
    if ( $anchortype eq 'popuplink' ) {
        $event =
'return foswiki.JSPopupPlugin.openPopupSectional(event, \'popupSection'
          . $popupSectionNumber
          . '\');';    #ASSUME onclick
        $output =
          $query->a( { href => $fallbackurl, onclick => $event }, $anchor );
    }
    elsif ( $anchortype eq 'anchorless' ) {
    }
    else {
        $event =
'onclick="return foswiki.JSPopupPlugin.openPopupSectional(event, \'popupSection'
          . $popupSectionNumber
          . '\');"';    #ASSUME onclick
        if ( $anchortype eq 'onmouseover' ) {
            $event =
'onmouseover="return foswiki.JSPopupPlugin.DelayedOpenPopupSectional(event, \'popupSection'
              . $popupSectionNumber
              . '\');return false;" onmouseout="return foswiki.JSPopupPlugin.CancelOpenPopup();"';
        }
        $output .= '<span ' . $event . '>' . "\n" . $anchor . "\n" . '</span>';
    }
...
sub registerRESTHandler {
    if ( $Foswiki::Plugins::VERSION eq 1.025 ) {
        my ( $name, $funcRef ) = @_;
        $FoswikiCompatibility{RESTHandlers}{ $pluginName . '.' . $name } =
          $funcRef;
    }
    else {
...
sub fakeFoswiki4RestHandlers {
    my ( $text, $topic, $web ) =
      @_;    #params passed on from beforeCommonTagsHandler
             #This is the view script based REST Handler cludge
    my $query    = Foswiki::Func::getCgiQuery();
    my $restCall = $query->param('rest');
    if (   defined($restCall)
        && defined( $FoswikiCompatibility{RESTHandlers}{$restCall} ) )
    {
        my $function = $FoswikiCompatibility{RESTHandlers}{$restCall};
        print $query->header( -content_type => 'text', );
        no strict 'refs';
        my $session = {};
...
"Version mismatch between $pluginName and Plugins.pm (tested on Cairo and Foswiki-4.0))"
        );
        return 0;
    }
